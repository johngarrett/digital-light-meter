...
bool BH1750::begin(Mode mode, byte addr, TwoWire* i2c) {
  Serial.print("[BH1750] began with mode=");
  Serial.print(mode);
  Serial.print(" addr=");
  Serial.print(addr);
  Serial.print(" i2c exists?=");
...
bool BH1750::configure(Mode mode) {
  ...
  case BH1750::ONE_TIME_LOW_RES_MODE:
    ...
    //__wire_write((uint8_t)BH1750_POWER_ON);
    I2C->write((uint8_t)BH1750_POWER_ON);
    res = I2C->endTransmission();
    Serial.print("Power on res:");
    Serial.println(res);
    _delay_ms(120);
    */
